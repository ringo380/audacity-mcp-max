...
- `describe_capability` reported loudness availability from a snapshot taken at import while reporting the numpy and scipy flags separately, so the verdict and the flags beside it could disagree. It derives the verdict from the two probes now. The capability tests that were meant to cover this asserted `LOUDNESS_AVAILABLE == (HAVE_NUMPY and HAVE_SCIPY)` - the implementation line restated against itself, which passes just as well with an `or` when both extras are installed - and skipped the empty-reason assertion entirely on any install without the extra. They now drive all four combinations. (docs/design/2026-07-24-measurement-verification-core.md)
- `dynamic_range_db` had no test that would notice it collapsing to zero. Every percentile assertion in the suite read a value near an end of the distribution, where interpolating and taking the nearest value agree, so replacing the interpolation with "always the minimum" passed the whole metrics suite. Covered now, along with the interpolation itself. (docs/design/2026-07-24-measurement-verification-core.md)
